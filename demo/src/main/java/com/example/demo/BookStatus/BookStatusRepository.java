...
public class BookStatusRepository {
    ...
    public BookStatusEntity findByBookId(Long aLong) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            ...
            BookStatusEntity b =  rs.next() ? new BookStatusEntity(rs.getLong(1), rs.getLong(2), rs.getLong(3), rs.getString(4)) : null;
            connection.close();
            return b;
    ...
    public List<BookStatusEntity> findByBookIdAndStatus(Long bookId, String status) throws SQLException {
        Connection connection = DataBase.getConnection();
        List<BookStatusEntity> bookStatusEntities = new ArrayList<>();
        try (PreparedStatement statement = connection.prepareStatement(
                "select * from book_status where book_id = ? and status = ?"
        )) {
            statement.setLong(1, bookId);
            statement.setString(2, status);
            var rs = statement.executeQuery();
